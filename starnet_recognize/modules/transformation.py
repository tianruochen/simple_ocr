...
class TPS_SpatialTransformerNetwork(nn.Module):
    ...
    def __init__(self, F, I_size, I_r_size, I_channel_num=1):
        ...
        self.F = F    #20
        self.I_size = I_size     #I_size=(32, 200)
        self.I_r_size = I_r_size  # = (I_r_height, I_r_width)  I_r_size=(32, 200)
        self.I_channel_num = I_channel_num     #I_channel_num=1

        #预测基准点的网络，其forword函数返回预测的基准点坐标 （batch，F，2）
...
class LocalizationNetwork(nn.Module):
    ...
    def __init__(self, F, I_channel_num):
        ...
        self.F = F  # 20
        self.I_channel_num = I_channel_num   # 1
...
class GridGenerator(nn.Module):
    ...
    def __init__(self, F, I_r_size):
        ...
        self.eps = 1e-6
        self.I_r_height, self.I_r_width = I_r_size   #（32，200）
        self.F = F      # 20

        #""" Return coordinates of fiducial points in I_r; C """
        self.C = self._build_C(self.F)  # F x 2

        # 在修正后的图片上画栅格，返回格子的坐标。self.P : ( self.I_r_width x self.I_r_height) x 2
        self.P = self._build_P(self.I_r_width, self.I_r_height)
        self.device = torch.device('cuda')
        ## for multi-gpu, you need register buffer  注册一个持久状态的缓冲区
        # 就是在模型中定义一个常量，同时，模型存储和加载时可以写出和读入
    ...
    def _build_inv_delta_C(self, F, C):   # C的shape：（F，2）
    ...
    def _build_P(self, I_r_width, I_r_height):  #200，32
    ...
    def _build_P_hat(self, F, C, P):
        # F：基准点个数 20
        # C：修正后图片上基准点坐标 F*2
        # P：修正后图片上画的栅格 self.P : ( self.I_r_width x self.I_r_height) x 2
    ...
    def build_P_prime(self, batch_C_prime):
        ...
        # batch_C_prime 变换网络预测的基准点的值  [batch_size x F x 2]
        batch_size = batch_C_prime.size(0)

        # （F+3）x（F+3）--》[batch_size x (F+3）x（F+3）]
        batch_inv_delta_C = self.inv_delta_C.repeat(batch_size, 1, 1)

        # n x F+3 --> batch_size x n x F+3
