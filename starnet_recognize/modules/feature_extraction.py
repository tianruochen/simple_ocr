...
class ResNet(nn.Module):
    ...
    def __init__(self, input_channel, output_channel, block, layers):
        # input_channel = 1   output_channel = 512  block = BasicBlock   layers = [1,2,5,3]
    ...
    def forward(self, x):
        ...
        # print(x.size())  torch.Size([19, 1, 32, 200])
        x = self.conv0_1(x)
        x = self.bn0_1(x)
        x = self.relu(x)
        x = self.conv0_2(x)
        ...
        # print(x.size())   torch.Size([19, 64, 32, 200])
        x = self.maxpool1(x)
        x = self.layer1(x)
        x = self.conv1(x)
        x = self.bn1(x)
        ...
        # print(x.size())   torch.Size([19, 128, 16, 100])
        x = self.maxpool2(x)
        x = self.layer2(x)
        x = self.conv2(x)
        x = self.bn2(x)
        ...
        # print(x.size())   torch.Size([19, 256, 8, 50])
        x = self.maxpool3(x)
        x = self.layer3(x)
        x = self.conv3(x)
        x = self.bn3(x)
        ...
        # print(x.size())    torch.Size([19, 512, 4, 51])
        x = self.layer4(x)
        x = self.conv4_1(x)
        x = self.bn4_1(x)
        x = self.relu(x)
        ...
        #print(x.size())    torch.Size([19, 512, 1, 51])
        return x
