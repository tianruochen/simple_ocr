...
def _topk(scores, K=40):
    ...
    #对每一个batch，每一个类别，都获得其topk的得分以及其索引（bctch，cat，K）
    topk_scores, topk_inds = torch.topk(scores.view(batch, cat, -1), K)

    #将topk的索引转换成其在热度图中的位置坐标
    topk_inds = topk_inds % (height * width)
    topk_ys = (topk_inds / width).int().float()    #（bctch，cat，K）
    topk_xs = (topk_inds % width).int().float()

    #对每一个batch，无关类别，获得其topk的得分以及其索引（batch，K）
    topk_score, topk_ind = torch.topk(topk_scores.view(batch, -1), K)
    topk_clses = (topk_ind / K).int()
    # 无关类别，返回一个一张图片上的topk的索引
    topk_inds = _gather_feat(
        topk_inds.view(batch, -1, 1), topk_ind).view(batch, K)
    topk_ys = _gather_feat(topk_ys.view(batch, -1, 1), topk_ind).view(batch, K)
    topk_xs = _gather_feat(topk_xs.view(batch, -1, 1), topk_ind).view(batch, K)

    #返回一张图片上topk的得分、索引、所属类别、纵坐标、横坐标  均为（batch，K）
    return topk_score, topk_inds, topk_clses, topk_ys, topk_xs


#根据预测的热度图，中心偏移量，宽高 获得Topk的预测结果  （batch，K，6）
#6维的向量包括：4维的坐标，1维的得分，1维的类别
def ctdet_decode(heat, wh, reg=None, cat_spec_wh=False, K=100):
    batch, cat, height, width = heat.size()  # [1, 1, 128, 128]
    ...
    if reg is not None:
        reg = _transpose_and_gather_feat(reg, inds)   #(batch,K,channel=2)
    ...
    detections = torch.cat([bboxes, scores, clses], dim=2)     ##(Batch,K,6)
